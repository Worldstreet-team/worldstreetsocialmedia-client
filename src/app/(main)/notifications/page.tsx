        ...
        <div className="flex flex-col min-h-screen">
            <header className="sticky top-0 z-20 bg-white/80 backdrop-blur-md border-b border-border-gray">
                ...
                <div className="flex">
                    {['all', 'verified', 'mentions'].map((tab) => (
                        ...
                        >
                            <span className={`text-[15px] capitalize ${activeTab === tab ? 'font-bold' : 'font-medium text-text-light'}`}>
                                {tab}
                            </span>
                            {activeTab === tab && (
            ...
            <div className="flex flex-col">
                {mockNotifications.map((notification) => (
                    <article key={notification.id} className="p-4 border-b border-border-gray hover:bg-black/[0.03] transition-colors cursor-pointer flex gap-3">
                        <div className="w-8 flex justify-end">
                            {notification.type === 'like' && <span className="material-symbols-outlined text-[#F91880] filled-icon">favorite</span>}
                            {notification.type === 'follow' && <span className="material-symbols-outlined text-primary filled-icon">person</span>}
                            {notification.type === 'mention' && <span className="material-symbols-outlined text-primary filled-icon">alternate_email</span>}
                            {notification.type === 'repost' && <span className="material-symbols-outlined text-[#00BA7C] filled-icon">repeat</span>}
                        </div>
                        <div className="flex flex-col gap-2 flex-1">
                            <div className="w-10 h-10 rounded-full bg-cover bg-center" style={{ backgroundImage: `url('${notification.user.avatar}')` }}></div>
                            <div>
                                <span className="font-bold hover:underline cursor-pointer">{notification.user.name} </span>
                                <span className="text-text-primary text-[15px]">
                                    {notification.type === 'follow' ? 'followed you' : notification.content}
                                </span>
                            </div>
                            {notification.type === 'mention' && (
                                <p className="text-text-light text-[15px] mt-1">
                                    Hey <span className="text-primary">@alexrivera</span>, checking out the new designs! Great work!
                                </p>
                            )}
                        </div>
                    </article>
