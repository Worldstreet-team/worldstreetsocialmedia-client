...
export function LeftSidebar() {
    ...
    return (
        <header className="w-[275px] hidden md:flex flex-col sticky top-0 h-screen px-2 overflow-y-auto">
                    ...
                    sidebarList.map((item, index) => {
                        ...
                        return (
                            <Link
                                ...
                                href={item.link}
                            >
                                <span className="inline-flex w-10 h-10 items-center justify-center"><item.icon isActive={isActive} /></span>
                                <span className={`text-lg pr-4 ${isActive ? "font-bold" : "font-semibold"}`}>{item.title}</span>
                            </Link>
                        );
                    })
                }
            ...
            <button className="mt-8 cursor-pointer bg-black text-white font-bold py-3.5 rounded-full text-[17px] shadow-sm transition-all mt-4 w-[85%]">
                Post
            </button>
