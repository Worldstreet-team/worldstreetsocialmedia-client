...
/**
 * The MARKET wordmark, vendored from tsionark.com along with its glitch.
 *
 * The wordmark is solid ink and inherits currentColor, so a card sets it to
 * white on stone or near-black on paper. The colour in the lockup belongs to
 * "Square", not here.
 *
 * The two clipped bands reproduce the site's torn-slice glitch with the same
 * geometry (y=14 h=16, y=46 h=18) and the same 6s cycle; the timing arrays
 * live in globals.css under "MARKET wordmark glitch".
 *
 * Note which glyphs move: M, E and T only. A, R, K and the comet trail sit in
 * one static path. That is the source's own split, verified by x-range (M is
 * 16-102, E is 377-446, T is 448-520), so do not "tidy" the three paths into
 * one, or the letter-level flicker is lost.
 */
 ...
 *
 * The wordmark is solid ink and only "Square" carries the animating gradient,
 * which is what makes the pairing read as one brand rather than two words in
 * two treatments.
 */
export function MarketSquareLockup({
	...
	return (
		// items-center centres the wordmark, which is the wider of the two;
		// self-start then drops "Square" to its left edge.
		<span className="inline-flex flex-col items-center">
			<MarketMark className={markClassName} />
			<span className={`mk-square mt-1 self-start ${wordClassName}`}>
				Square
			</span>
		</span>
	);
